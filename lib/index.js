    ...
    if (this.cache) {
        ...
        for (let i = 0; i < segments.length; ++i) {
            ...
            for (let j = 0; j < keys.length; ++j) {
                const key = keys[j];
                Bt.clearTimeout(this.cache[segment][key].timeoutId);
            }
        }
    }
...
internals.Connection.prototype.set = function (key, value, ttl, callback) {
    ...
    if (ttl > Number.MAX_SAFE_INTEGER) {                                                         // Math.pow(2, 53) - 1
        return callback(new Error(`Invalid ttl (greater than ${Number.MAX_SAFE_INTEGER})`));
    }
    ...
    if (cachedItem && cachedItem.timeoutId) {
        Bt.clearTimeout(cachedItem.timeoutId);
